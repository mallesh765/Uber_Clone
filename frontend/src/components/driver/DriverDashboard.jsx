...
const DriverDashboard = () => {
  ...
  const fetchDriverData = () => {
    ...
    fetch(`http://localhost:8000/drivers/dashboard`, {
      headers: {
  ...
  return (
    <>
        ...
        >
          ...
          {reject ? null : (
            ...
            >
              <Text>Name : {}</Text>
              <Text>Email : {}</Text>
              <Text bg="white">Origin Location : {}</Text>{" "}
              <Text>Destination Location : {}</Text>
              <Text>
                {/* Fare : {Math.ceil(driverData.distanceNumber * driverData.mul)} */}
              </Text>
      ...
      <Heading textAlign={"center"}>
        There's an Uber ride for everyone
      </Heading>
      <Grid
        templateColumns={{
